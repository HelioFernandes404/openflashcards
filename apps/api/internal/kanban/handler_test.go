...
func setupRouter(t *testing.T, svc kanbanServicer) (*gin.Engine, *auth.JWTManager, uuid.UUID) {
	t.Helper()
...
func TestCreateKanbanCardRequiresAuth(t *testing.T) {
	r, _, _ := setupRouter(t, newFakeKanbanService())
	body, _ := json.Marshal(map[string]any{"title": "Card 1"})
